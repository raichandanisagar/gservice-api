...
class Bigquery:
    ...
    def _build_dataframe_from_query_response(query_response):
        data = []
        schema_fields = [schema_field['name'] for schema_field in query_response['schema']['fields']]
        schema_fields
        ...
        df = pd.DataFrame(data=data,columns=schema_fields)
        return df
    
    
    def fetch_query_results(self, query, query_params=None, legacy_sql=False, timeout=300, dryrun=False):
        """Submit query to bigquery and return results as a dataframe
        Args:
            ...
            timeout: float, Time period for query completion (seconds); 300 secs (5 mins) by default
            dryrun: bool, if True query does not run instead provides details on bytes processed and cache hit
        
        Returns dict 
            job_complete, error_message, job_id, total_bytes_processed and pandas dataframe of rows"""
    ...
    def get_dataset(self, dataset_id):
        """Returns the dataset resource specified by the dataset_id
        for more details: https://developers.google.com/resources/api-libraries/documentation/bigquery/v2/python/latest/bigquery_v2.datasets.html#get
        Args:
